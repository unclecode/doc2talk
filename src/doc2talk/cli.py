...
from .core import ChatSession, SessionManager, ChatEngine
...
class ChatCLI:
    ...
    async def display_streaming_response(self, session: ChatSession, engine: ChatEngine, question: str) -> str:
    ...
    async def list_sessions_cmd(self):
        ...
        for session in SessionManager.list_sessions():
            table.add_row(
                session["id"], 
                session["created"].split("T")[0] if "T" in session.get("created", "") else "", 
    ...
    async def chat_session(self, engine: ChatEngine, session: ChatSession):
        ...
        while True:
            try:
                ...
                response = await self.display_streaming_response(session, engine, user_input)
